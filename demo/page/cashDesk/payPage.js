...
//获取银行限额
var getBankLimit = function(bankCode, cb){
    $http.post({
...
//获取默认支付方式
var resoleBankList = function( data, cb ) {
    ...
    if( banklist ) {
        for( var i = 0; i < banklist.length; i++ ) {
            ...
            if( bankInfo.isDefault == 1 ) {
                ...
                getBankLimit(bankCode, function(obj){
                    cb(bankInfo, obj);
                });
            }
        }
...
var PayPage = React.createClass({
    ...
    onPress : function() {
        if( !this.state.agreeFlg ) {
            React.Alert.alert('请确认勾选协议');
            return;
        }
        var _this = this;
        getSkbKey();
        getDefaultPay({}, function( info, text ){
            _this.setState({
